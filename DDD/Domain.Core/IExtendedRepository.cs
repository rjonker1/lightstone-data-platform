...
namespace LightstoneApp.Domain.Core
{
        ...
        where TEntity : class
    {
        ...
        /// <returns>List of selected elements</returns>
        IEnumerable<K> GetPagedElements<K, S>(int pageIndex, int pageCount, Expression<Func<K, S>> orderByExpression,
            bool ascending)
            where K : TEntity;
        ...
        /// <param name="ascending">Specify if order is ascending</param>
        /// <returns>List of selected elements</returns>
        IEnumerable<K> GetFilteredElements<K, S>(Expression<Func<K, bool>> filter,
