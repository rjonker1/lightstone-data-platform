...
namespace PackageBuilder.Domain.EventHandlers.DataProviders
{
    public class DataProviderCreatedHandler : AbstractMessageHandler<DataProviderCreated>
    {
        ...
        {
            _repository.Save(new DataProvider(command.Id, command.Name, command.Description, command.CostPrice, 1, command.Owner, command.CreatedDate, command.EditedDate));
        }
    }
}
