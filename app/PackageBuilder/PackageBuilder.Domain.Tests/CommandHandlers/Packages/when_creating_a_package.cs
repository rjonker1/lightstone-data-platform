...
namespace PackageBuilder.Domain.Tests.CommandHandlers.Packages
{
    public class when_creating_a_package : when_not_persisting_entities
    {
        ...
        {
            base.Observe();
            var command = new CreatePackage(Guid.NewGuid(), "Name", "Description", 10d, 20d, "Notes", new[] { IndustryMother.Automotive }, StateMother.Draft, "Owner", DateTime.UtcNow, null, new List<IDataProviderOverride> { DataProviderOverrideMother.Ivid }.AsEnumerable());
