...
namespace PackageBuilder.Api.Modules
{
    public class PackageModule : NancyModule
    {
                                        ...
                                        INEventStoreRepository<Package> writeRepo, IRepository<State> stateRepo)
        {
            ...
            {
                return Response.AsJson(new { Response = new []{ writeRepo.GetById(parameters.id) } });
            };
            ...
            Post["/Package/Add"] = parameters =>
            {
                ...
                bus.Publish(new CreatePackage(Guid.NewGuid(), dto.Name, dto.Description, dto.CostOfSale, dto.RecommendedSalePrice, stateResolve.FirstOrDefault(), dto.Owner, DateTime.Now, null, dProviders));
            ...
            Post["/Package/Edit/{id}"] = parameters =>
            {
                ...
                bus.Publish(new UpdatePackage(parameters.id, dto.Name, dto.Description, dto.CostOfSale, dto.RecommendedSalePrice, stateResolve.FirstOrDefault(), dto.Version, dto.Owner, dto.CreatedDate, DateTime.Now, dProviders));
