    ...
    public class DataFieldMap : ICreateAutoMapperMaps
    {
        public void CreateMaps()
        {
            ...
            Mapper.CreateMap<DataProviderFieldItemDto, IDataField>()
                .ConvertUsing(s => 
                    new DataField(s.Name, s.Label, s.Definition, s.Industries, Convert.ToDouble(s.Price), Convert.ToBoolean(s.IsSelected)));//, Type.GetType(s.Type)));
            Mapper.CreateMap<IEnumerable<DataProviderFieldItemDto>, IEnumerable<IDataField>>()
                .ConvertUsing(x => 
                    x.Select(Mapper.Map<DataProviderFieldItemDto, IDataField>));
