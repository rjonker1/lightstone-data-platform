...
namespace Monitoring.Dashboard.UI.Core.Models.DataProvider
{
  ...
  select count(m.Id) as RequestPerDataProviderCount, m.DataProviderId from DataProviderEventLog m where m.CommandTypeId = 6 and DataProviderId <> 6 and m.CommitStamp >= DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()), 0) AND m.CommitStamp < DATEADD(MONTH, DATEDIFF(MONTH, 0, GETDATE()) + 1, 0) group by m.DataProviderId, m.DataProviderId
