...
namespace Monitoring.Dashboard.UI.Core.Models
{
    ...
    {
        private const string GetErrors = @"select count([state]) ErrorCount, RequestId from [Billing].[dbo].[Transaction] where ([StateId] not in (1,7)) and RequestId in @RequestIds group by RequestId";
