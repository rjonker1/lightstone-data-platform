...
{
    public class FinanceRepository : IReadOnlyRepository
    {
        private const string CacheIp = "127.0.0.1:6379";
        private static readonly ILog Log = LogManager.GetLogger<FinanceRepository>();
        ...
        public IEnumerable<TItem> Get<TItem>(string sql, object param) where TItem : class
        {
            try
            {
                using (var connection = DatabaseConnectionFactory.FinacedInterestsConnection)
