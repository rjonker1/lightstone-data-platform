...
namespace Lace.Toolbox.PCubed.Deserialization
{
    public class PCubedJsonDeserializer : IDeserializer
    {
        ...
        private object ConvertValue(Type type, object value)
        {
            ...
            // HACK: (Jean) Custom PCubed bool values
            if (type == typeof(bool))
            {
                if (stringValue.Contains("Yes")) return true;
                if (stringValue.Contains("No")) return false;
