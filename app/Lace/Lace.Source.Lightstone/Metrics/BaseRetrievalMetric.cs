...
namespace Lace.Source.Lightstone.Metrics
{
    public class BaseRetrievalMetric : IRetrieveValuationFromMetrics
    {
        ...
        {
            return new LastFiveSalesMetric(_getSales.Sales, _getMuncipalities.Municipalities).MetricResult;
        }

        private IEnumerable<IRespondWithEstimatedValueModel> GetEstimatedValues()
        {
            return new EstimatedValuesMetric(_request, _getStatistics.Statistics).MetricResult;
        }
        ...
        {
            return new RepairIndexMetric(_request, _getStatistics.Statistics, _getBands.Bands).MetricResult;
        }

        private IEnumerable<IRespondWithAuctionFactorModel> GetAuctionFactors()
        {
            return new AuctionFactorsMetric(_request, _getStatistics.Statistics, _getMakes.Makes).MetricResult;
        }

        private IEnumerable<IRespondWithAreaFactorModel> GetAreaFactors()
        {
            return new AreaFactorsMetric(_getStatistics.Statistics, _getMuncipalities.Municipalities).MetricResult;
        }

        private IEnumerable<IRespondWithAmortisedValueModel> GetAmortisedValues()
        {
            return new AmortisedValueMetric(_request, _getStatistics.Statistics, _getBands.Bands).MetricResult;
        }
