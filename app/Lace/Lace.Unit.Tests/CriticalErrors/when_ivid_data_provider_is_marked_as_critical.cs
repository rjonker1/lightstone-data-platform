...
{
    public class when_ivid_data_provider_is_marked_as_critical : Specification
    {
        ...
        public when_ivid_data_provider_is_marked_as_critical()
        {
            _command = MonitoringBusBuilder.ForIvidCommands(Guid.NewGuid());
            _request = new[] {new IvidFailRequest()};
        ...
        [Observation]
        public void then_critical_flag_should_be_raised_on_error()
        {
            ...
            _response.OfType<IProvideDataFromIvid>().First().HasCriticalFailure.ShouldBeTrue();
            _response.OfType<IProvideDataFromIvid>().First().CriticalFailureMessage.ShouldNotBeEmpty();
        }
    }
}