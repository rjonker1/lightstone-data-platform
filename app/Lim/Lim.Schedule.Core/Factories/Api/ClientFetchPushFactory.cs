...
{
    public class ClientFetchPushFactory : AbstractFetchFactory<FetchConfigurationForClientCommand, IEnumerable<ApiPushIntegration>>
    {
        ...
        public ClientFetchPushFactory(IRepository repository, IPush<ApiInitializePushCommand> pusher)
        {
            _repository = repository;
            _pusher = pusher;
        }

        public override IEnumerable<ApiPushIntegration> Fetch(FetchConfigurationForClientCommand command)
        {
            try
            {
                ...
                .Select(s => new ApiPushIntegration(s.Key, s.Id, new ApiConfigurationIdentifier(s.BaseAddress, s.Suffix,
                    ...
                    new IntegrationContractIdentifier(
                        (AutoMapper.Mapper.Map<IEnumerable<IntegrationContract>, IEnumerable<IntegrationContractDto>>(
                            _repository.Get<IntegrationContract>(w => w.Configuration.Id == s.Id && w.Contract == contractId && w.IsActive))).Select(
                                c => c.Contract)),
